...
#define WALK_VERIFY                                                            \
  5 // verify that all active nodes have been
    // correctly processed
...
int is_active(uint data) { return (data >= ActiveThreshold); }
...
uint load_nodes(char *filename, int *mode, uint *cutoff, uint *range,
                uint **nodes) {
...
int main(int argc, char **argv) {
    ...
    while ((c = getopt(argc, argv, "N:c:f:C:s:F:v:M:h")) != -1)
      ...
      case 'h':
        printf(
            ...
            "\tv :  the value to be used to generate nodes with constant "
            "value\n"
  ...
  if (actual_nodes != N)
    printf("shame on me! %u nodes instead of %u have been found!", actual_nodes,
           N);
  if (faults > 0)
    printf("a tragedy occured.. %u nodes are unsorted!!\n", faults);

  printf(" > counting active nodes.. ");
  fflush(stdout);
  timing = CPU_TIME;
  ...
  if (case_to_run < USE_TASKS) {
    ...
    printf("\t * processing the active nodes.. ");
    fflush(stdout);
...
#pragma omp parallel for schedule(dynamic, chunk)
    ...
    printf("\t * generating the tasks and processing the array.. ");
    fflush(stdout);
    timing = CPU_TIME;
#pragma omp parallel
    {
