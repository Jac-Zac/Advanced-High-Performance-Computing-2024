...
#pragma omp taskyield
  ...
  dbgout("[ %llu ] T %d V %d locked: (p: %d p>n: %d) (n: %d n<p: %d)\n",
         ...
         ((prev != NULL) && (prev->next != NULL) ? (prev->next)->data : -1),
         (next != NULL ? next->data : -1),
         ((next != NULL) && (next->prev != NULL) ? (next->prev)->data : -1));
...
#pragma omp atomic update
    ...
    if ((prev != NULL) && (prev->next != next)) {
      ...
      dbgout("[ %llu ]\t>> T %d V %d next has changed: from %d to %d\n",
             TIMESTAMP, me, value, (next != NULL ? next->data : -1),
             (prev->next != NULL ? (prev->next)->data : -1));
    ...
    // note that next can not be NULL
    {
      ...
      dbgout("[ %llu ]\t>> T %d V %d prev has changed: from %d to %d\n",
             TIMESTAMP, me, value, (prev != NULL ? prev->data : -1),
             (next->prev != NULL ? (next->prev)->data : -1));
      ...
      printf("Some serious error occurred, a prev = next = NULL situation "
             "arose!\n");
      return -3;
    }
  }
...
#pragma omp task
  ...
  if (actual_nodes != N)
    printf("shame on me! %d nodes instaed of %d have been found!", actual_nodes,
           N);
