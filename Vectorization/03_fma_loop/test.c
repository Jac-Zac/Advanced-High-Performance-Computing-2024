...
void __attribute__((noinline)) warmup(double *restrict A, double *restrict B,
...
double __attribute__((noinline)) process(const double *restrict A,
...
#define VD_SIZE 4
  ...
  // Just do the sum of the final results
  v4df_u *vsum_u = (v4df_u *)&vsum;
  vsum_u->v[0] += vsum_u->v[1] + (vsum_u->v[2] + vsum_u->v[3]);

  // NOTE: Important doing the loop reminding
