...
void MandelbrotCPU(float x1, float y1, float x2, float y2, int width,
                   int height, int maxIters, unsigned short *image) {
  float dx = (x2 - x1) / width;
  float dy = (y2 - y1) / height;
  for (int j = 0; j < height; ++j)
    for (int i = 0; i < width; ++i) {
...
int main(int argc, char **argv) {
  ...
  switch (mode) {
  case 0:
    MandelbrotCPU(x1, y1, x2, y2, width, height, maxIters, image);
    break;
  case 1:
    MandelbrotCPU_Vectorizable(x1, y1, x2, y2, width, height, maxIters, image);
    break;
  case 2:
    MandelbrotCPU_FullyVectorizable(x1, y1, x2, y2, width, height, maxIters,
                                    image);
    break;
  }
  ...
  printf("Pixels NOT in the Mandelbrot set: %d / %d (%.1f%%)\n", escaped_count,
         width * height, 100.0 * escaped_count / (width * height));
