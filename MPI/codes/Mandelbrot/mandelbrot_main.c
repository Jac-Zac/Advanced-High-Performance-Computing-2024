...
int main(int argc, char **argv) {
  ...
  if (rank == 0) {
    ...
    printf("Bounds: [%.3f, %.3f] x [%.3f, %.3f]\n", ctx.bounds.x_min,
           ctx.bounds.x_max, ctx.bounds.y_min, ctx.bounds.y_max);
    printf("Output: %s\n\n", output_file);
  }
  ...
  for (int r = 0; r < num_procs; r++) {
    if (rank == r) {
      printf("  Rank %d: %d patches, %.3f s compute time\n", rank,
             patches_processed, t_work_end - t_work_start);
    }
    MPI_Barrier(MPI_COMM_WORLD);
  }
  ...
  if (rank == PRODUCER_RANK) {
    ...
    memcpy(image_copy, ctx.image, ctx.img_width * ctx.img_height * sizeof(int));
