...
static int lock_init(Context *ctx) {
  ...
  ret = MPI_Win_create(queue_size, sizeof(Patch), MPI_INFO_NULL, MPI_COMM_WORLD,
                       ctx->queue_buffer, &ctx->queue_win);
  if (ret != MPI_SUCCESS)
    return ret;

  ret =
      MPI_Win_allocate(counter_size, sizeof(int64_t), MPI_INFO_NULL,
                       MPI_COMM_WORLD, ctx->queue_counters, &ctx->counter_win);
  if (ret != MPI_SUCCESS)
    return ret;

  ret = MPI_Win_allocate(image_size, sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD,
                         ctx->image, &ctx->image_win);
  if (ret != MPI_SUCCESS)
    return ret;
...
static int lock_enqueue(Context *ctx, const Patch *patch) {
  ...
  MPI_Fetch_and_op(&(int64_t){1}, &tail, MPI_INT64_T, PRODUCER_RANK, TAIL,
                   MPI_SUM, ctx->counter_win);

  /* Also increment pending counter */
  MPI_Accumulate(&(int64_t){1}, 1, MPI_INT64_T, PRODUCER_RANK, ACTV, 1,
                 MPI_INT64_T, MPI_SUM, ctx->counter_win);
  ...
  MPI_Put(patch, sizeof(Patch), MPI_BYTE, PRODUCER_RANK, slot, sizeof(Patch),
          MPI_BYTE, ctx->queue_win);
...
static int lock_dequeue(Context *ctx, Patch *patch) {
  int64_t head, tail;
  int success = 0;
  int attempts = 0 const int max_empty_attempts =
      1000; /* Prevent neverending spinning */
  ...
  while (!success) {
    ...
    MPI_Win_lock(MPI_LOCK_EXCLUSIVE, PRODUCER_RANK, 0, ctx->counter_win);
    MPI_Compare_and_swap(&new_head, &head, &result, MPI_INT64_T, PRODUCER_RANK,
                         0, ctx->counter_win);
    MPI_Win_unlock(PRODUCER_RANK, ctx->counter_win);
    ...
    if (result == head) {
      ...
      MPI_Get(patch, sizeof(Patch), MPI_BYTE, PRODUCER_RANK, slot,
              sizeof(Patch), MPI_BYTE, ctx->queue_win);
      MPI_Win_unlock(PRODUCER_RANK, ctx->queue_win);
...
static int lock_write_results(Context *ctx, int x, int y, int size,
  ...
  for (int row = 0; row < actual_height; row++) {
    ...
    MPI_Put(&values[row * actual_width], actual_width, MPI_INT, PRODUCER_RANK,
            img_offset, actual_width, MPI_INT, ctx->image_win);
  }
...
static void lock_finalize(Context *ctx) {
  ...
  if (ctx->queue_win != MPI_WIN_NULL)
    MPI_Win_free(&ctx->queue_win);
  if (ctx->counter_win != MPI_WIN_NULL)
    MPI_Win_free(&ctx->counter_win);
  if (ctx->image_win != MPI_WIN_NULL)
    MPI_Win_free(&ctx->image_win);
...
QueueBackend backend_lock = {.init = lock_init,
